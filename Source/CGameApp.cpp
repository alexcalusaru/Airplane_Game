...
bool CGameApp::CreateDisplay()
{
	...
	USHORT			Width			= GetSystemMetrics(SM_CXFULLSCREEN);
	USHORT			Height = GetSystemMetrics(SM_CXFULLSCREEN);
	RECT			rc;
	WNDCLASSEX		wcex;
	...
	switch (Message)
	{
		...
		case WM_KEYDOWN:
			...
			case VK_RETURN:
				...
				_Player2->Explode();
				break;
				// Case to save the game by pressing P
			case 'P':
				MessageBox(hWnd, "Game is saved.", "Save Game", MB_OK);
				SaveGame();
				break;
				// Case to load the game by pressing L
			case 'L':
				MessageBox(hWnd, "Game is loaded.", "Load Game", MB_OK);
				LoadGame();
				break;
				// Cast to rotate the plane by pressing R
			case 'R':

				if (_Player1->GetDir() == 0)
					_Player1->RotatePlaneRight();

				else if (_Player1->GetDir() == 1)
					_Player1->RotatePlaneDown();

				else if (_Player1->GetDir() == 2)
					_Player1->RotatePlaneLeft();

				else if (_Player1->GetDir() == 3)
					_Player1->RotatePlaneUp();
				else
					break;
			}
...
bool CGameApp::BuildObjects()
{
	...
	_Player1 = new CPlayer(_Buffer, "data/JW.bmp");
	_Player2 = new CPlayer(_Buffer, "data/caine.bmp");
...
void CGameApp::SaveGame()
{
	...
	myfile << _Player1->Position().x << " " << _Player1->Position().y << "\n";
	myfile.close();
}
// Function to LoadTheGame
void CGameApp::LoadGame()
{
	std::ifstream myfile("Save-Load.txt");
	double posx, posy;
	myfile >> posx >> posy;
	_Player1->Position() = Vec2(posx, posy);
